...
<svelte:head>
	<title>{data.profileUser.username}</title>
</svelte:head>

<header class="sticky top-0 z-50 border-b border-gray-200 bg-white/90 backdrop-blur">
	<div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
		<div>
			<h1 class="text-3xl font-black text-gray-900">{data.profileUser.username}</h1>
			<p class="mt-1 text-sm text-gray-500">User profile</p>
		</div>

		<div class="flex items-center gap-3">
			{#if data.isOwner}
				<a href={resolve(`/user/${data.profileUser.id}/new`)} class="rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
					Add new
				</a>
				<form action="/logout?/logout" method="POST">
					<button type="submit" class="rounded-full bg-red-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-red-600">
						Logout
					</button>
				</form>
			{/if}

			<a href={resolve('/')} class="rounded-full bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-700">
				Home
			</a>
		</div>
	</div>
...
<main class="mx-auto max-w-7xl px-4 py-8">
	{#if !data.isOwner}
		<div class="mb-6 rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-600">
	...
	<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
		{#each data.images as image (image.id)}
			<article class="relative overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
				<a href={resolve(`/user/${image.author_id}/images/${image.id}`)} class="absolute inset-0 z-0" aria-label={`Open post by ${image.username}`}></a>
				...
				<div class="relative z-10 pointer-events-none space-y-4 px-4 py-4">
					...
					<div class="flex flex-wrap items-center justify-between gap-3">
						<form method="POST" action="?/upvoteImage" class="pointer-events-auto flex items-center gap-3">
							...
							<button type="submit" class="rounded-full px-4 py-2 text-sm font-semibold text-white transition {data.viewer && image.viewer_voted ? 'bg-gray-500 hover:bg-gray-600' : 'bg-red-500 hover:bg-red-600'}">
						...
						<div class="flex items-center gap-3">
							...
							{#if data.isOwner}
								<form action="?/deleteImage" method="POST" use:enhance class="pointer-events-auto">
									<input type="hidden" name="id" value={image.id} />
